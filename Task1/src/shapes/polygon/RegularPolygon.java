...
public class RegularPolygon extends Polygon {
	...
	public RegularPolygon(Point theCenter, Point pointOnCircle, int sideNum, int frameWidth, Color frameColor,
			Color fillColor) {
	...
	private List<Point> getPolygonPoints(Point theCenter, Point pointOnCircle, int sideNum) {
		...
		double radius = Math
				.sqrt(Math.pow((pointOnCircle.x) - theCenter.x, 2) + Math.pow(pointOnCircle.y - theCenter.y, 2));
		double z;
	...
	public void setPointOnCircle(Point pt) {
		setPoints(getPolygonPoints(getLocation(), pt, sideNum));
	}
